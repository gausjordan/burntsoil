...
function debugDrawPoints() {
    canvCtx1.fillStyle = "rgba(0,0,255,255)";
    for (let i=0; i < controlPoints.length; i++) {
        canvCtx1.fillRect(controlPoints[i].x, controlPoints[i].y, 6, 6);
    }
}
...
function debugDrawCurves() {
    ...
    canvCtx1.fillStyle = "rgba(255,0,0,160)";
    ...
    for (let seg = 0; seg < controlPoints.length-3; seg+=1) {
        ...
        let string = "rgb(" + c1 + "," + c2 + "," + c3 + ")";
           canvCtx1.fillRect(controlPoints[seg+0].x, controlPoints[seg+0].y, 6, 6);
        ...
        for (let t = 0; t < 1; t += step/4) {
                ...
                canvCtx1.fillRect( Math.round(x), Math.round(y), 1, 30);
        }
    ...
    for (let seg = 0; seg < controlPoints.length-3; seg+=1) {
        ...
        for (let t = 0; t < 1; t += step) {
                ...
                let y = cubicInterpolate(controlPoints[seg+0].y, controlPoints[seg+1].y, controlPoints[seg+2].y, controlPoints[seg+3].y, t);
                y = y + y*step;
                canvCtx1.fillRect( Math.round(px), Math.round(y), 1, 30);
                px = px + 1;
        }
        // px += controlPoints[seg+1].x-controlPoints[seg+0].x;
    }
